...
static void help()
{
    printf("program       featureFile labelFile DTParamFile max_check dist_threhold saveFile\n");
    printf("BT_DTC_train  feature.mat label.mat param.txt   2         -1       model.txt\n");
    printf("feature file: .mat file has a 'feature' variable. \n");
    printf("label file:   .mat file has a 'label' variable. \n");
    printf("max_check: backtracking number. \n");
    printf("dist_threshold: positive value --> threshold, negative value for INT_MAX.");
}
...
int main(int argc, const char * argv[])
{
    ...
    BTDTCTreeParameter tree_param;
    bool is_read = tree_param.readFromFile(tree_param_file);
    assert(is_read);
    
    
    BTDTClassifierBuilder builder;
    builder.setTreeParameter(tree_param);
    
    BTDTClassifier model;
    builder.buildModel(model, features, labels, valid_features, valid_labels,
                       max_check, dist_threshold, save_file);
